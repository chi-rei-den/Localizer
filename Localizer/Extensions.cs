    ...
    public static class Extensions
    {
        public static void Import(this ModTranslation modTranslation, BaseEntry entry, CultureInfo culture)
        {
            if (entry != null)
            {
                ...
                {
                    Localizer.Log.Warn($"Dismatch origin text when importing \"{modTranslation.Key}\", Origin in mod: {modTranslation.GetDefault()}, Origin in package: {entry.Origin}");
                }
        ...
        public static string DefaultOrEmpty(this ModTranslation modTranslation)
        {
            ...
            if (string.IsNullOrWhiteSpace(d) || d == modTranslation.Key)
                return "";
            return d;
        }
