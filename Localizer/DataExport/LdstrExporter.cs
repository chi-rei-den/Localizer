...
namespace Localizer.DataExport
{
    public class LdstrExporter : Exporter
    {
        ...
        protected override File Extract()
        {
            ...
            var asm = Config.Package.Mod.Code;

            var file = new LdstrFile();
            var config = (LdstrExportConfig) Config;
        ...
        protected LdstrEntry GetEntryFromMethod(MethodInfo method)
        {
            ...
            entry.Instructions = new List<BaseEntry>();
        ...
        {
            var oldE = oldEntry as LdstrEntry;
            var newE = newEntry as LdstrEntry;
            ...
            foreach (var newIns in newE.Instructions)
            {
                ...
                logger.TextUpdateLog($"New instruction of {key}: [{newIns}]");
            }
        }
    }
}